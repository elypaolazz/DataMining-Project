...
def baseline_method(data, patient_id, cond_id):
    ...
    # check if patients and condition exist
    if patient_id not in patients_df.id.values:
        ...
        if cond_id not in conditions_df.id.values:
            ...
            # build whole patient utility matrix and calculate cosine similarity with patient id matrix
            cosine_sim = []
            for i in tqdm(patients_df.id.values, desc="Caclulating cosine distances"):
                cured_u_mat = utility_matrix(i, data).to_numpy()
                cosine_sim_pat = cosine_similarity(patient_id_u_mat.flatten(), cured_u_mat.flatten())
                cosine_sim.append(cosine_sim_pat)
            
            cos_sim_df = pd.DataFrame({"pat_cured" : patients_df.id.values, 
                "cosine_similarity": cosine_sim})
            cos_sim_df = cos_sim_df[cos_sim_df.pat_cured.isin(pat_cured)]
